...
def Score(topRank=10, asClass=False):
    ...
    class Score0to(Method):
        ...
        def honBallot(cls, utils):
            ...
            bot = min(utils)
            scale = max(utils) - bot
            return [floor((cls.topRank + 0.99) * (util - bot) / scale) for util in utils]
            ...
            if cuts[0] == cuts[1]:
                ...
                strat = [
                    max(
                        ...
                        min(
                            cls.topRank,
                            floor((cls.topRank + 0.99) * (util - cuts[1]) / (cuts[0] - cuts[1])),
                        ),
                    )
                    for util in voter
                ]
...
class Mav(Method):
    ...
    def stratBallotFor(self, polls):
        ...
        def stratBallot(cls, voter):
            ...
            if stratGap == 0:
                strat = extraStrat = [(4 if (util >= frontUtils[0]) else 0) for util in voter]
                isStrat = True
                ...
                cutoffs = [
                    (
                        (min(frontUtils[0], self.baseCuts[i]))
                        if (i < floor(targResult))
                        else (
                            (frontUtils[1])
                            if (i < floor(frontResult) + 1)
                            else min(top, self.baseCuts[i])
                        )
                    )
                ...
                extraStrat = [
                    max(
                        ...
                        min(
                            10,
                            floor(4.99 * (util - frontUtils[1]) / (frontUtils[0] - frontUtils[1])),
                        ),
                    )
                    for util in voter
                ]
            return dict(strat=strat, extraStrat=extraStrat, isStrat=isStrat, stratGap=stratGap)
...
class Irv(Method):
    ...
    def eliminateCandidate(self, inputPrefs, toEliminate):
        ...
        for ranking, votes in inputPrefs.items():
            newranking = [candidate for candidate in ranking if candidate != toEliminate.candidate]
    ...
    def candidateVotes(self, prefSchedule):
        ...
        return (
            sorted(candidates.values(), key=lambda c: (c.votes, c.candidate), reverse=True)
            + alternates
        )
...
class V321(Mav):
    ...
    def results(self, ballots, isHonest=False, **kwargs):
        ...
        if upset > 0:
            ...
            r2s[runnerUp], r2s[top] = r2s[top] - 0.125, r2s[runnerUp] + 0.125
        r2s[top] = max(r2s[top], r2s[runnerUp] + 0.5)
        if isHonest:
            upset2 = sum(
                sign(ballot[semifinalists[o1s[0]]] - ballot[semifinalists[o1s[2]]])
                for ballot in ballots
            )
            self.__class__.extraEvents["3beats1"] = upset2 > 0
            upset3 = sum(
                sign(ballot[semifinalists[o1s[0]]] - ballot[semifinalists[o1s[1]]])
                for ballot in ballots
            )
            ...
            if len(o2s) > 3:
                ...
                fourthWin = (
                    fourthNotLasts > n1s[o1s[1]]
                    and sum(
                        sign(ballot[fourth] - ballot[semifinalists[o1s[2]]]) for ballot in ballots
                    )
                    > 0
                )
            ...
            for j in range(n):
                if i != j:
                    if beatStrength[i][j] > beatStrength[j][i]:
                        numWins[i] += 1
                    if (
                        beatStrength[i][j] == beatStrength[j][i] and i < j
