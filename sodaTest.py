...
class ElectionCounts:
    ...
    def delegated(self, amounts, cantWin=None):
        ...
        delg = list(self.delg)
        delg[delegator] = 0
        result = ElectionCounts(
            delg,
            appr + self.appr,
            self.prefs,
            self.order[1:],
            cantWin or self.cantWin,
            self.majSmith,
        )
        result.matrix = (
            self.matrix - self.oneMatrix(dprefs, self.delg[delegator]) + self.appMatrix(appr)
        )
        return result
        ...
        for amounts in self.possibleDelegations(worstWinnerIndex, idealWinnerIndex):
            ...
            if i < bestHopeIndex:
                if verbose > 2 - len(self.order) * 1.0 / 10:
                    print(
                        "updating w,len(self.order),amounts",
                        w,
                        i,
                        len(self.order),
                        amounts,
    ...
    def possibleDelegations(self, worstWinnerIndex, idealWinnerIndex):
        ...
        if self.minWin:
            for i in range(
                idealWinnerIndex + 1, worstWinnerIndex
            ):  # if i==3 and idealWinnerIndex==1 we want [max, max, mid, mid, 0, 0]
    ...
    def scores(self):
        ...
        for i in range(len(self.delg)):
            for j in range(self.n):
                scores[self.prefs[i][j]] = scores[self.prefs[i][j]] + self.delg[i] * (
                    self.n - j - 1
                )
        for j in range(self.n):
...
myEc2 = ElectionCounts(
    [5, 30, 20, 0],
    [35, 0, 0, 1],
    [[0, 1, 2, 3], [1, 2, 0, 3], [2, 1, 0, 3], [3, 2, 1, 0]],
    [0, 1, 2, 3],
)


myEc3 = ElectionCounts(
    [4, 3, 2, 0],
    [0, 0, 0, 0],
    [[0, 1, 2, 3], [2, 3, 0, 1], [3, 0, 1, 2], [3, 2, 1, 0]],
    [0, 1, 2, 3],
)
